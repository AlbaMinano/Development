...
for i = 1:N
    zi(i,1) = exp(-(sigmaU^2)/2).*ui(i,1); %% Get zi 
end 
...
for a = 1:A
    for i = 1:N
        nonSeasonal(i, :, a) = exp(-(sigmaEpsilon^2)/2).*epsilon(i,:, a);
    end
end

for month = 1:T 
    eGmHigh(month,1) = exp(gmHigh(month));
    eGmMiddle(month,1) = exp(gmMiddle(month));
    eGmLow(month,1) = exp(gmLow(month));
end
...
for age = 1: A
    for month = 1: T
        for household = 1: N 
            ciHigh(household, month, age) = beta^(month-1).*log(ciHigh(household, month, age)); 
            ciLow(household, month, age) = beta^(month-1).*log(ciLow(household, month, age)); 
            ciMiddle(household, month, age) = beta^(month-1).*log(ciMiddle(household, month, age)); 
        end
    end
end
...
for age = 1: A
    for month = 1: T
        for household = 1: N 
            ciNonSeasonal(household, month, age) = beta^(month-1).*log(ciNonSeasonal(household, month, age)); 
        end
    end
end
...
%% Question 1 part 1 : we compute the welfare costs removing the seasonal risk
...
for household= 1:N
    gHigh(household,1) = exp((wZiNonSeasonal(household)-wZiHigh(household))/(sumBetaAge*sumBetaMonth))-1;
    gLow(household,1) = exp((wZiNonSeasonal(household)-wZiLow(household))/(sumBetaAge*sumBetaMonth))-1;
    gMiddle(household,1) = exp((wZiNonSeasonal(household)-(wZiMiddle(household)))/(sumBetaAge*sumBetaMonth))-1;
end

hold on
h1 = histogram(gLow, 10)
hold on
h2 = histogram(gMiddle, 15)
hold on
h3 = histogram(gHigh, 20)
legend( [h1 h2 h3],{'Low', 'Medium', 'High'})
hold off
%% Question 1 part 1: Welfare costs of removing the non seasonal consumption risk 
% We repeat the same as before but without seasonality. 
...
for age = 1: A
    for month = 1: T
        for household = 1: N 
            ciHomHigh(household, month, age) = beta^(month-1).*log(ciHomHigh(household, month, age)); 
            ciHomLow(household, month, age) = beta^(month-1).*log(ciHomLow(household, month, age)); 
            ciHomMiddle(household, month, age) = beta^(month-1).*log(ciHomMiddle(household, month, age)); 
        end
    end
end
...
for household= 1:N
    gHighHom(household,1) = exp((wZiHighHom(household)-wZiHigh(household))/(sumBetaAge*sumBetaMonth))-1;
    gLowHom(household,1) = exp((wZiLowHom(household)-wZiLow(household))/(sumBetaAge*sumBetaMonth))-1;
    gMiddleHom(household,1) = exp((wZiMiddleHom(household)-wZiMiddle(household))/(sumBetaAge*sumBetaMonth))-1;
end
...
for age = 1: A
    for month = 1: T
        for household = 1: N 
            ciPerHigh(household, month, age) = beta^(month-1).*log(ciPerHigh(household, month, age)); 
            ciPerLow(household, month, age) = beta^(month-1).*log(ciPerLow(household, month, age)); 
            ciPerMiddle(household, month, age) = beta^(month-1).*log(ciPerMiddle(household, month, age)); 
        end
    end
end
...
for household= 1:N
    gHighPer(household,1) = exp((wZiHighPer(household)-wZiHigh(household))/(sumBetaAge*sumBetaMonth))-1;
    gLowPer(household,1) = exp((wZiLowPer(household)-wZiLow(household))/(sumBetaAge*sumBetaMonth))-1;
    gMiddlePer(household,1) = exp((wZiMiddlePer(household)-wZiMiddle(household))/(sumBetaAge*sumBetaMonth))-1;
end
