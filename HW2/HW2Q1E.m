...
gmLow = [ - 0.073, -0.185, 0.071, 0.066, 0.045, 0.029, 0.018, 0.018, 0.001, -0.017, -0.041, - 0.04]'; 
...
for age = 1: A
    for month = 1 : T
        for household = 1: N 
            ciHighHigh(household, month, age) = (ciHighHigh(household, month, age).^(1-eta)) /(1-eta); 
            ciHighLow(household, month, age) = (ciHighLow(household, month, age).^(1-eta)) /(1-eta); 
            ciHighMiddle(household, month, age) = (ciHighMiddle(household, month, age).^(1-eta)) /(1-eta); 
            
            ciLowHigh(household, month, age) = (ciLowHigh(household, month, age).^(1-eta)) /(1-eta); 
            ciLowMiddle(household, month, age) = (ciLowMiddle(household, month, age).^(1-eta)) /(1-eta); 
            ciLowLow(household, month, age) = (ciLowLow(household, month, age).^(1-eta)) /(1-eta); 
            
            ciMiddleHigh(household, month, age) = (ciMiddleHigh(household, month, age).^(1-eta)) /(1-eta); 
            ciMiddleLow(household, month, age) = (ciMiddleLow(household, month, age).^(1-eta)) /(1-eta); 
            ciMiddleMiddle(household, month, age) = (ciMiddleMiddle(household, month, age).^(1-eta)) /(1-eta); 
...
t = zeros(N, 1);
 ...
 for household = 1: N 
     gHighHigh(household,1) = (round(wZiNonSeasonal(household))/round(wZiHighHigh(household)))^(1/(1-eta)) -1;
     gHighLow(household,1) = (round(wZiNonSeasonal(household))/round(wZiHighLow(household)))^(1/(1-eta)) -1;
     gHighMiddle(household,1) = (round(wZiNonSeasonal(household))/round(wZiHighMiddle(household)))^(1/(1-eta)) -1;
     
     gLowHigh(household,1) = (round(wZiNonSeasonal(household))/round(wZiLowHigh(household)))^(1/(1-eta)) -1;
     gLowLow(household,1) = (round(wZiNonSeasonal(household))/round(wZiLowLow(household)))^(1/(1-eta)) -1;
     gLowMiddle(household,1) = (round(wZiNonSeasonal(household))/round(wZiLowMiddle(household)))^(1/(1-eta)) -1;
     
     gMiddleHigh(household, 1) = (round(wZiNonSeasonal(household))/round(wZiMiddleHigh(household)))^(1/(1-eta)) -1;
     gMiddleLow(household, 1) = (round(wZiNonSeasonal(household))/round(wZiMiddleLow(household)))^(1/(1-eta)) -1;
     gMiddleMiddle(household, 1) = (round(wZiNonSeasonal(household))/round(wZiMiddleMiddle(household)))^(1/(1-eta)) -1;
...
for age = 1: A
    for month = 1 : T
        for household = 1: N 
            ciHomHighHigh(household, month, age) = (ciHomHighHigh(household, month, age).^(1-eta)) /(1-eta); 
            ciHomHighLow(household, month, age) = (ciHomHighLow(household, month, age).^(1-eta)) /(1-eta); 
            ciHomHighMiddle(household, month, age) = (ciHomHighMiddle(household, month, age).^(1-eta)) /(1-eta); 
            
            ciHomLowHigh(household, month, age) = (ciHomLowHigh(household, month, age).^(1-eta)) /(1-eta);
            ciHomLowLow(household, month, age) = (ciHomLowLow(household, month, age).^(1-eta)) /(1-eta);
            ciHomLowMiddle(household, month, age) = (ciHomLowMiddle(household, month, age).^(1-eta)) /(1-eta);
            
            ciHomMiddleHigh(household, month, age) = (ciHomMiddleHigh(household, month, age).^(1-eta)) /(1-eta); 
            ciHomMiddleLow(household, month, age) = (ciHomMiddleLow(household, month, age).^(1-eta)) /(1-eta); 
            ciHomMiddleMiddle(household, month, age) = (ciHomMiddleMiddle(household, month, age).^(1-eta)) /(1-eta); 
...
for household = 1 :N
 ...
 for household = 1: N 
     gHighHighHom(household,1) = (round(wZiHighHighHom(household))/round(wZiHighHigh(household)))^(1/(1-eta)) -1;
     gHighLowHom(household,1) = (round(wZiHighLowHom(household))/round(wZiHighLow(household)))^(1/(1-eta)) -1;
     gHighMiddleHom(household,1) = (round(wZiHighMiddleHom(household))/round(wZiHighMiddle(household)))^(1/(1-eta)) -1;
     
     gLowHighHom(household,1) = (round(wZiLowHighHom(household))/round(wZiLowHigh(household)))^(1/(1-eta)) -1;
     gLowLowHom(household,1) = (round(wZiLowLowHom(household))/round(wZiLowLow(household)))^(1/(1-eta)) -1;
     gLowMiddleHom(household,1) = (round(wZiLowMiddleHom(household))/round(wZiLowMiddle(household)))^(1/(1-eta)) -1;
     
     gMiddleHighHom(household, 1) = (round(wZiMiddleHighHom(household))/round(wZiMiddleHigh(household)))^(1/(1-eta)) -1;
     gMiddleLowHom(household, 1) = (round(wZiMiddleLowHom(household))/round(wZiMiddleLow(household)))^(1/(1-eta)) -1;
     gMiddleMiddleHom(household, 1) = (round(wZiMiddleMiddleHom(household))/round(wZiMiddleMiddle(household)))^(1/(1-eta)) -1;
