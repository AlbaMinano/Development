...
egen id = group(hh year)
duplicates tag id, gen(tag)
 
split wave, p("-")
destring wave1, replace

gen year2 = year
replace year = wave1 if tag ==1


//Check 

 egen id2 = group(hh year)
duplicates tag id2, gen(tag2)

replace year = wave1 if tag2==1 
*/
...
xi: reg log_income age age_sq i.wave familysize  i.female i.ethnic  i.rural
predict res_inc, res
...
by region year urban, sort: egen cons_agg = total(ctotal)
gen logConsAgg = log(cons_agg)
sort hh year
by hh: gen growth_agg = logConsAgg-logConsAgg[_n-1]
...
//Merge with consumption growth
merge m:1 region urban year using "$temp/growthCons.dta"
keep if _merge == 3
drop _merge

keep region year urban hh logConsAgg inctotal growth growth_inc  rural growth_agg

save "$temp/dataclean.dta", replace 
statsby _b, by(hh) saving("$temp/q1.dta", replace):  reg growth growth_inc growth_agg, nocons vce(robust)

use "$temp/q1.dta", clear
...
graph export "$output/Q1A.png", width(1080) as(png) replace
...
graphregion(color(white)) 
graph export "$output/Q1A_2.png", width(1080) as(png) replace
...
merge 1:1 hh using "$temp/q1.dta",
drop _merge
...
xtitle("Beta", size(medlarge)) ///
graphregion(color(white)) 
graph export "$output/Q2a.png", width(1080) as(png) replace
...
merge 1:m hh using "$temp/dataclean.dta"
keep if _merge == 3
...
Question 3
*************************/
 use "$temp/dataclean.dta", replace
...
outreg2 using "$output/Table1a.tex", tex replace ctitle( "$\triangle$" "($\log$)" "$\c_{it}$")/*
*/nocons keep(growth_inc growth_agg) see label adjr2 addtext(Sample, Overall)
