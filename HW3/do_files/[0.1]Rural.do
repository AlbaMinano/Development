...
merge m:1 region year using "$temp/growthConsrural.dta"
keep if _merge == 3
drop _merge

keep region year urban hh logConsAgg inctotal growth growth_inc  rural growth_agg

save "$temp/datacleanrural.dta", replace 
statsby _b, by(hh) saving("$temp/q1rural.dta", replace):  reg growth growth_inc growth_agg, nocons vce(robust)

use "$temp/q1rural.dta", clear
...
graph export "$output/Q1Arural.png", width(1080) as(png) replace
...
graphregion(color(white)) 
graph export "$output/Q1A_2rural.png", width(1080) as(png) replace
...
use "$temp/datacleanrural.dta", clear
...
merge 1:1 hh using "$temp/q1rural.dta",
drop _merge
...
xtitle("Beta", size(medlarge)) ///
graphregion(color(white)) 
graph export "$output/Q2arural.png", width(1080) as(png) replace
...
merge 1:m hh using "$temp/datacleanrural.dta"
keep if _merge == 3
...
Question 3
*************************/
 use "$temp/datacleanrural.dta", replace
...
*/nocons keep(growth_inc growth_agg) see label adjr2 addtext(Sample, Rural)
